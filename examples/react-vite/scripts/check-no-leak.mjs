#!/usr/bin/env node
// Layer-1 dev-only guard regression check for examples/react-vite.
// Fails the build if any widget-chain symbol appears in the production
// dist/ bundle. The substring "agent-devtools" alone is intentionally
...
// devtools-widget identifier and would create a false positive on this
// example. The list below is restricted to symbols that originate only
// in this monorepo's adapter/transport/picker chain.
