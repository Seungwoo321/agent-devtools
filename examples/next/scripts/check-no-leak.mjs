#!/usr/bin/env node
// Layer-1 dev-only guard regression check for examples/next.
...
for (const sub of SCAN_SUBDIRS) {
  ...
  for await (const file of walk(subDir)) {
    ...
    // Skip Next's source maps for server-only edge runtimes — they can include
    // string forms of import paths in webpack module ids that match the
    // substring "agent-devtools" without containing the symbol identifiers we
    // actually guard. Symbol-based scan handles both server and static.
    scanned += 1;
