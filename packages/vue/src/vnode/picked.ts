...
export interface DescribePickedVueOptions {
...
/**
 * Build a `PickedEvidence` for a DOM element rendered by Vue 3.
 *
 * Same evidence shape the React adapter emits — the widget UI and the
 * server prompt formatter are framework-agnostic, so a Vue picker just
 * needs to fill the same fields using Vue's ComponentInternalInstance
 * graph instead of React fibers. When an element was not rendered by
 * Vue (static HTML, third-party widget), only the DOM-derived fields
 * are populated and componentChain comes back empty.
 */
export function describePickedVue(
  element: Element,
  options: DescribePickedVueOptions = {},
): PickedEvidence {
...
function collectComponentChain(
  instance: ComponentInstanceLike | null,
  maxDepth: number,
...
/**
 * Serialise the picked component's resolved props. We prefer
 * `instance.props` (the public, resolved props) over `instance.attrs`
 * because `attrs` includes inherited bindings the user didn't author.
 * Same replacer strategy as the React adapter: drop functions, DOM
 * nodes, vnodes, circulars.
 */
function serialiseProps(instance: ComponentInstanceLike | null, limit: number): string | undefined {
...
function isVnodeLike(v: unknown): boolean {
  if (!v || typeof v !== 'object') return false;
  const candidate = v as { __v_isVNode?: unknown; component?: unknown };
  return candidate.__v_isVNode === true || candidate.component != null;
}
