...
describe('createRelatedImportsFetcher', () => {
  ...
  it('forwards a derived AbortSignal so the internal timeout can interrupt the fetch', async () => {
    ...
    await fetcher('src/X.tsx', callerController.signal);
  ...
  it('returns [] when the internal timeout fires before the dev server responds', async () => {
  ...
  it('aborts the in-flight fetch when the caller signal fires', async () => {
    ...
    const pending = fetcher('src/Picked.tsx', caller.signal);
    caller.abort();
