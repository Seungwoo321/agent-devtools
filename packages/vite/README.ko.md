...
- **어댑터-무관** — 위젯 import 대상은 `framework` / `importFrom` 옵션으로 변경할 수 있습니다. `framework: 'auto'` 는 호스트 `package.json` 을 읽어 대응되는 `@agent-devtools/<framework>` 어댑터 (`react`, `vue`, `vue2`, `next`, `next-pages`, `nuxt`, `nuxt2`, `angular`, `svelte`, `sveltekit`) 를 선택하며, 기본값은 `@agent-devtools/react` 입니다.
...
| `framework`      | `'auto' \| 'react' \| 'vue' \| 'vue2' \| 'next' \| 'next-pages' \| 'nuxt' \| 'nuxt2' \| 'angular' \| 'svelte' \| 'sveltekit'` | `'auto'`               | mount 할 어댑터. `'auto'` 는 호스트 `package.json` 을 읽어 우선순위 (`sveltekit` > `nuxt`/`nuxt2` > `next` > `angular` > `svelte` > `vue`/`vue2` > `react`) 로 결정하고, 매칭이 없으면 `react` 로 fallback. `next-pages` 는 auto-detect 되지 않으며 명시적으로 지정해야 합니다. |
