...
- **Shadow root (`closed`)** — 위젯은 host element `<agent-devtools-widget>`
  ...
  들여다볼 수 없다 (`packages/react/src/widget/shadow-root.ts:80`).
...
- **Element picker overlay 는 shadow root 밖.** picker 가 hover 요소 위에 그리는
  ...
  있게 한다 (`packages/react/src/picker/overlay.ts:1`).
- **Dual React tree.** 위젯 UI 는 호스트 앱의 React root 와 별개의 root 안에서
  ...
  올린다 (`packages/react/src/orchestrator/mount.ts:156`).
...
- **드래그 이동 + 영구화.** 버튼을 누른 채로 드래그하면 위치를 옮길 수 있고,
  ...
  다음 mount 때 같은 자리에서 다시 떠오른다 (`launcher.ts:108`). 창 크기가
...
- **클릭 동작.** orchestrator 가 `composer.element.style.display === 'none'`
  ...
  바로 입력 가능한 상태로 만든다 (`packages/react/src/orchestrator/mount.ts:254`).
...
- **추가 액션.** 컴포저 헤더에는 picker 토글, settings (톱니바퀴), terminal
  ...
  으로 서버측 ACP 세션을 새로 발급한다 (`mount.ts:332`).
...
- **Permission Mode** — `requestPermission` 콜백에 대한 일괄 정책. 다섯 가지:
  ...
  - `bypassPermissions` — 모든 권한 요청 무조건 허용. 위험도가 높아 settings
    ...
    (`packages/react/src/settings/types.ts:10`,
    `packages/react/src/settings/panel.ts:118`). 행 자체가 빨간 배경으로
    강조된다 (`settings/panel.ts:374`).
  - `plan` — 읽기 전용 plan 모드.
...
- **Workspace Root** — 서버가 보고한 워크스페이스 절대경로 (`/v1/agent/info` 의
  ...
  확인하는 용도 (`settings/panel.ts:133`).

**영구화 범위.** provider 와 permissionMode 는 `localStorage` 키
`agent-devtools:settings` 에 JSON 으로 저장되어 다음 mount 까지 살아남는다
(`packages/react/src/settings/storage.ts:11`). 패널 크기 (composer drag-resize 결과)
...
`PageContext` 가 담는 필드 (`packages/react/src/context/types.ts:114`):
...
- `route` — `{ pathname, search, hash }`. router 와 무관하게 `window.location`
  에서 추출한다 (`packages/react/src/context/route.ts:9`).
- `pageFiles` — 현재 페이지의 React fiber tree 를 walk 해서 모은 component
  ...
  root 부터 fiber 를 따라간다 (`packages/react/src/context/build.ts:66`).
- `errors` — `createErrorObserver()` 가 mount 시점부터 수집해 둔 콘솔 에러/예외
  레코드의 최근 50개 (`mount.ts:237`).
...
- **State machine.** picker 는 `idle → active → picked → idle` 의 3-state
  순수 reducer 위에서 돈다 (`packages/react/src/picker/state.ts:8`). active
...
- **Click 으로 확정.** active 상태에서의 click 은 `preventDefault` +
  ...
  만들고 composer 의 picked chip 으로 노출한다 (`mount.ts:240`).
