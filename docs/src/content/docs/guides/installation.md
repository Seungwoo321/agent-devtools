...
2. **Node.js 22.12 LTS (Jod) 이상.** Node 24+ 에서도 동작한다.
   `node --version` 으로 확인.
...
> **어떤 걸 고를지 모르겠다면 React + Vite 로 시작한다.** canonical
> first-run 경로다 — 가장 짧은 dev 루프, 가장 두텁게 커버된 어댑터, 그리고
> docs 의 모든 다른 예제가 이 스택을 기준으로 정렬되어 있다. 익숙한 스택에서
> 위젯이 파일 한 번 수정하는 걸 본 다음에 다른 어댑터로 옮겨가도 된다.
...
- **위젯 아이콘이 안 보임** → [문제 해결: 위젯이 dev 서버에서 아예 보이지 않음](/guides/troubleshooting/#위젯이-dev-서버에서-아예-보이지-않음)
- **`501 agent stream not configured`** → [문제 해결: Claude Code CLI handshake 실패](/guides/troubleshooting/#claude-code-cli-handshake-실패-acp-차일드-spawn-직후-즉시-끊김)
