...
![브라우저 위젯·루프백 dev 서버·로컬 Claude Code Agent SDK 를 한 루프로 — 픽커가 클릭을 PickedEvidence 로 환원하고, 페어링 토큰은 127.0.0.1 위에서 Authorization 헤더로만 흐르며, Edit/Write 가 파일에 적용되면 HMR 이 같은 탭에서 반영한다](/how-it-works-ko.png)
