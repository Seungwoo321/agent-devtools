...
- **Shadow root (`closed`)** — The widget attaches a single host element
  ...
  `element.shadowRoot` (`packages/react/src/widget/shadow-root.ts:80`).
- **Launcher / composer / settings live inside the shadow root.** The
  ...
  (`packages/react/src/widget/shadow-root.ts:57`).
- **The element picker overlay lives outside the shadow root.** The outline
  ...
  (`packages/react/src/picker/overlay.ts:1`).
- **Dual React tree.** The widget UI renders inside a React root that is
  ...
  onto its own shadow root (`packages/react/src/orchestrator/mount.ts:156`).
...
- **Position and size.** The default position is `{ x: 24, y: 24 }` away
  ...
  (`packages/react/src/launcher/launcher.ts:27`).
- **Drag-to-move + persistence.** Holding the button and dragging lets you
  ...
  reappears in the same spot (`launcher.ts:108`). At mount time the saved
  position is clamped against the current viewport once more, so it never
  ends up off-screen after a window resize.
...
- **Click behavior.** The orchestrator checks the current visibility via
  ...
  (`packages/react/src/orchestrator/mount.ts:254`).
- **The composer follows the launcher.** During a drag, `onPositionChange`
  ...
  bottom edge sits 16px above the launcher (`mount.ts:262`).
...
- **Default size and anchor.** The panel is 320px wide and 420px tall by
  ...
  (`packages/react/src/composer/composer.ts:75`).
- **Keyboard behavior.**
  ...
  - `Escape` → close the composer only (launcher stays)
    (`packages/react/src/composer/composer.ts:341`).
...
- **In-flight UI state.** When the transport starts replying it calls
  ...
  a new submit happens (`mount.ts:282`).
...
- **Extra actions.** The composer header has buttons for the picker toggle,
  ...
  hand out a fresh server-side ACP session (`mount.ts:332`).
...
(`packages/react/src/settings/panel.ts:1`).
...
- **Provider** — which runtime handles the next prompt.
  ...
  (`packages/react/src/settings/panel.ts:152`).
...
- **Permission Mode** — the blanket policy for the `requestPermission`
  ...
  - `bypassPermissions` — unconditionally allow every permission request.
    ...
    composer (`packages/react/src/settings/types.ts:10`,
    `packages/react/src/settings/panel.ts:118`). The row itself is
    highlighted with a red background (`settings/panel.ts:374`).
...
- **Workspace Root** — the absolute workspace path reported by the server
  ...
  (`settings/panel.ts:133`).

**Persistence scope.** Provider and permissionMode are serialised as JSON
under the `localStorage` key `agent-devtools:settings` and survive across
mounts (`packages/react/src/settings/storage.ts:11`). The panel size (the
...
(`packages/react/src/orchestrator/mount.ts:287`,
`packages/react/src/context/build.ts:33`).

Fields carried by `PageContext`
(`packages/react/src/context/types.ts:114`):
...
- `route` — `{ pathname, search, hash }`. Extracted from `window.location`
  ...
  (`packages/react/src/context/route.ts:9`).
- `pageFiles` — the list of component source files
  ...
  (`packages/react/src/context/build.ts:66`).
...
- **State machine.** The picker runs on a 3-state pure reducer:
  ...
  (`packages/react/src/picker/state.ts:8`). A click during the active
...
- **Hover behavior.** While active, every mousemove uses
  ...
  (`packages/react/src/picker/picker.ts:99`).
...
- **Escape to cancel.** Pressing `Escape` while the picker is active
  cancels it and returns to `idle` (`picker.ts:92`).
...
(`packages/react/src/context/picked.ts:47`,
`packages/react/src/context/types.ts:62`):
